...
launch_sites = np.append(launch_sites, 'All')
...
def pie(site_dropdown):
    ...
    if site_dropdown == "All":
        fig = px.pie(
            data_frame=spacex_df, names='Launch Site', values='class',
            title='Total success launches by site')
    else:
        title = f'Successful launches from site {site_dropdown}'
        filtered_df= spacex_df[spacex_df['Launch Site'] == site_dropdown]
        filtered_df = filtered_df.groupby(['Launch Site','class']).size().reset_index(name='class count')
        fig = px.pie(
            data_frame=filtered_df, names='class', values='class count',
            title=title)
    return fig
